...
if (!empty($data['id']) && !empty($data['product_name']) && !empty($data['quantity']) && !empty($data['purchasing_price']) && !empty($data['expiry_date']) && !empty($data['added_by'])) {
    try {
        // Database connection
        ...
        if (!$product_id) {
            throw new Exception("Product: $sold_by_username not found");
        }
